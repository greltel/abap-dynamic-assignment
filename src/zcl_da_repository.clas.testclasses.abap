...
CLASS ltc_repository IMPLEMENTATION.
  ...
  METHOD given_inactive_then_skipped.
    ...
    insert_variant( counter = '00001' is_active = abap_false ).
  ...
  METHOD given_two_keys_then_both.
    ...
    " given
    insert_variant( counter = '00003' ).
    insert_variant( counter = '00007' parameter = 'OTHER' ).
    insert_variant( counter = '00009' parameter = 'UNASKED' ).

    " when - the same key twice, as a request with two entities would send it
    DATA(counters) = cut->read_last_counters( VALUE #( ( progname = test_program parameterid = test_parameter )
                                                       ( progname = test_program parameterid = test_parameter )
                                                       ( progname = test_program parameterid = 'OTHER' ) ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = '00007'
        act = counters[ progname = test_program parameterid = 'OTHER' ]-counter
  ...
  METHOD given_delete_all_then_count.
    ...
    insert_variant( counter = '00001' ).
    insert_variant( counter = '00002' ).
    insert_variant( counter = '00001' parameter = 'OTHER' ).
