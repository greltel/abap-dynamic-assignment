...
"! Covers {@link ZCL_DA_OPTION_VH}, the query provider behind the options value help.
...
"! set of ten rows.</p>
CLASS ltc_option_vh DEFINITION FINAL FOR TESTING
  RISK LEVEL HARMLESS
  DURATION SHORT.

  PRIVATE SECTION.

    DATA cut      TYPE REF TO zcl_da_option_vh.
    ...
    CONSTANTS between_option TYPE zde_da_opt VALUE 'BT'    ##NO_TEXT.
    CONSTANTS other_element  TYPE string     VALUE `PROGNAME` ##NO_TEXT.
    CONSTANTS domain_values  TYPE i          VALUE 10.

    METHODS setup.

    " ----- the fixed values of the domain -----------------------------------
    "! Every fixed value of ZDO_DA_OPT must reach the value help.
    ...
    "! A filter on the options element must become a range.
    METHODS given_filter_then_range       FOR TESTING.
    "! A filter on another element must not narrow the options.
...
CLASS zcl_da_option_vh DEFINITION LOCAL FRIENDS ltc_option_vh.


CLASS ltc_option_vh IMPLEMENTATION.

  METHOD setup.
    cut      = NEW zcl_da_option_vh( ).
  ...
  METHOD given_domain_then_all_values.
    ...
    " when
    DATA(options) = cut->read_fixed_values( ).

    " then
    cl_abap_unit_assert=>assert_equals(
        exp = domain_values
        act = lines( options )
        msg = 'Every fixed value of ZDO_DA_OPT must reach the value help' ).
  ...
  METHOD given_domain_then_description.
    ...
    cl_abap_unit_assert=>assert_not_initial(
        act = VALUE #( options[ options = between_option ]-options_descr OPTIONAL )
  ...
  METHOD given_no_filter_then_empty.
    ...
    cl_abap_unit_assert=>assert_initial(
        act = range
        msg = 'Without a filter the provider must serve every option' ).
  ...
  METHOD given_filter_then_range.
    ...
    ranges = VALUE #( ( name  = zcl_da_option_vh=>element_options
                        range = VALUE #( ( sign = 'I' option = 'EQ' low = between_option ) ) ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        ...
        msg = 'A filter on the options element must become a range line' ).
  ...
  METHOD given_other_element_then_none.
    ...
    cl_abap_unit_assert=>assert_initial(
        act = range
        msg = 'A filter on another element must not narrow the options' ).
  ...
  METHOD given_no_data_then_not_set.
    ...
    response->set_data( VALUE zcl_da_option_vh=>ty_options( ) ).
